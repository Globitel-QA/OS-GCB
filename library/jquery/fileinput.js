...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            // noinspection HtmlUnknownAttribute
            tBtnBrowse = '<div tabindex="500" class="{css}" {status} id="tBtnBrowse">{icon} {label}</div><button id="tBtnViewSample" class="btn btn-success" onclick="info();return false;" tabindex="500">View Sample</button>';
        ...
        _showUploadError: function (msg, params, event) {
        	/*if(msg.indexOf("Are you sure to continue?") !==-1){}else{
             		AlertFunction(msg);
        	}*/
        ...
        _parseError: function (operation, jqXHR, errorThrown, fileName) {
            ...
            self.cancelling = false;
         // alert(errMsg);
            return fileName ? errMsg + '<b>' + fileName + ' </b> upload was aborted.'  : errMsg;
            /*Original return ==> return fileName ? '<b>' + fileName + ': </b>' + errMsg : errMsg;*/
        },
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {            	 
                ...
                if (self.maxFileSize > 0 && fileSize > self.maxFileSize) {
					...
					// caption).replace('{size}', fSizeKB)
                    // .replace('{maxSize}', self.maxFileSize);
                   // AlertFunction('Maximum allowed file size is 1.455 MB.');
                    self.isError = throwError(msg, file, previewId, i);
                    return;
                }
                if (self.minFileSize !== null && fileSize <= $h.getNum(self.minFileSize)) {
                	
                	// AlertFunction('File '+ caption+' is too small and must be
					// larger than <b>' + self.minFileSize +' KB</b>.');
                	msg = 'File cannot be empty, please select another file.';
                	AlertFunction(msg);            
                	self.reset();				 
                	// msg = self.msgSizeTooSmall.replace('{name}', caption).replace('{size}', fSizeKB).replace('{minSize}', self.minFileSize);
                	self.disableGlobitel(); // add from OdaiAhmed 3042019
                    self.isError = throwError(msg, file, previewId, i);                    
                    return;
                }
                ...
                if (fileCount === 0 && !$h.isEmpty(fileExt) && $h.isArray(fileExt) && !$h.isEmpty(fileExtExpr)) {
                    ...
                    if (fileCount === 0) {
                        ...
                        // msg = self.msgInvalidFileExtension.replace('{name}',
						// caption).replace('{extensions}', strExt);
                    	AlertFunction(jQuery.i18n.prop('Upload_AlertFileFormatisnotsupported'));
                        self.isError = throwError(msg, file, previewId, i);
                        return;
                    }
                }
        ...
        _change: function (e) {        	 
            ...
            if (!self.isUploadable && $h.isEmpty($el.val()) && self.fileInputCleared) { // IE
																						// 11
																						// fix
                self.fileInputCleared = false;
                return;
        ...
        clear: function () {
            var self = this, cap;
            $('.file-caption-name').removeClass('text-danger'); // Add From OdaiAhmed 152019 from validtion size that inculde in page upload
        ...
        disableGlobitel: function () { // Create 3042019 OdaiAhmed
            var self = this;
            self.isDisabled = true;
            self._raise('filedisabled');
            self.$element.attr('disabled', 'disabled');
        ...
        enableGlobitel: function () { // Create 3042019 OdaiAhmed
            var self = this;
            self.isDisabled = false;
            self._raise('fileenabled');
            self.$element.removeAttr('disabled');
    ...
    $.fn.fileinput.defaults = {
        ...
        buttonLabelClass: 'hidden-xs Translate',
        browseIcon: '<i class="glyphicon glyphicon-folder-open"></i>&nbsp;',
        browseClass: 'btn btn-primary',
        removeIcon: '<i class="glyphicon glyphicon-trash" id="btnDelete"></i>',
    ...
    $.fn.fileinputLocales.en = {
        ...
        removeLabel: jQuery.i18n.prop('btnRemove') || 'Remove',
        ...
        msgUploadAborted: jQuery.i18n.prop('Upload_File')  ,
        msgUploadThreshold: jQuery.i18n.prop('Upload_Processing')  ,
        msgUploadBegin: jQuery.i18n.prop('Upload_Initializing'),
        msgUploadEnd: jQuery.i18n.prop('Upload_Done') , 
        msgUploadEmpty: jQuery.i18n.prop('Upload_AlertSelectFile') ,
