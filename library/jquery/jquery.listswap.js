(function($) {
	$.fn.listswap = function(options) {
		...
		(settings.add_class) ? div_class_ = ' ' + settings.add_class
				: div_class_;
		$(this).wrapAll(
				"<div id='" + div_id_ + "' class='listboxswap" + rtl_class_
						+ div_class_ + "'></div>");
		...
		this
				.each(function() {
					...
					var options_count = $(div_id + " #" + select_id
							+ " > option").length;
					var truncate_class = '';
					var round_class = '';

					create_element(parent_wrapper, parent_element);
					(settings.truncate) ? truncate_class = ' class="truncate"'
							: truncate_class;
					(i % 2 !== 0) ? class_name = 'source_wrapper'
							: class_name = 'destination_wrapper';

					$('#' + listbox_id).addClass(class_name);

					if ($(this).attr('data-text')) {
						var text_data = '<p' + truncate_class + '>'
					...
					if ($(this).attr('data-search')) {
						...
						if (!$(this).attr('data-text')) {
							...
							$(div_id + ' .listbox_search').addClass(
									'listbox_round_class');
						}
					}
					...
					if (!$(this).attr('data-text')
							&& !$(this).attr('data-search')) {
						$(
								div_id + ' .source_wrapper ul, ' + div_id
										+ ' .destination_wrapper ul').addClass(
								'listbox_round_class');
					}

					$(div_id + " #" + select_id + " > option")
							.each(
									function() {
										...
										var element = '<li class="listbox_option" data-value="'
												+ value
												+ '"><span'
												+ truncate_class
												+ '>'
					...
					(i % 2 !== 0) ? source_search = $(this).attr('data-search')
		...
		if (settings.height) {
			$(
					div_id + ' .source_wrapper ul, ' + div_id
							+ ' .destination_wrapper ul').css('height',
					settings.height);
		}
		...
		if (settings.is_scroll === true) {
			$(
					div_id + ' .source_wrapper ul, ' + div_id
							+ ' .destination_wrapper ul').css('overflow-y',
					'scroll');
		...
		var controls = '<div class="listbox_controls">'
				+ '<ul>'
				+ '<li class="addAll" title="Add All"><button type="button" class="btn-xs btn   btn-default gray "><span class="fa fa-angle-double-right"></span></button></li>'
				+ '<li class="add" title="Add"><button type="button" class="btn-xs btn btn-outline btn-default "><span class="fa fa-angle-right"></span></button></li>'
				+ '<li class="remove" title="Remove"><button type="button" class="btn-xs btn btn-outline btn-default "><span class="fa fa-angle-left"></span></button></li>'
				+ '<li class="removeAll" title="Remove All"><button type="button" class="btn-xs btn btn-outline btn-default"><span class="fa fa-angle-double-left"></span></button></li>'
				+
		...
		$(
				div_id + ' .source_wrapper .listbox_option, ' + div_id
						+ ' .destination_wrapper .listbox_option').click(
				function() {
					$(this).toggleClass('selected');
				});

		$(div_id + ' .listbox_controls .add').click(
				function() {
					$(div_id + ' .source_wrapper .listbox_option.selected')
							.each(function() {
		...
		$(div_id + ' .listbox_controls .addAll').click(
				function() {
					$(div_id + ' .source_wrapper .listbox_option')
							.each(function() {
					add_remove_handlerAll($(this), div_id+ ' .destination_wrapper ul',source_select_id,destination_select_id,1);
									});
					refresh_list();
				});

		$(div_id + ' .listbox_controls .remove')
				.click(function() {
		...
		$(div_id + ' .listbox_controls .removeAll').click(
				function() {
					$(div_id + ' .destination_wrapper .listbox_option')
							.each(function() {
					add_remove_handlerAll($(this), div_id + ' .source_wrapper ul', destination_select_id,source_select_id,2);
									});
					refresh_list();
				});
		if (source_search) {
		...
		function add_remove_handler(target, destination, select_1, select_2) {
			...
			$(div_id + " select#" + select_2)
					.append($("<option></option>").attr("value", data_value).text(text));
			target.remove();
		}

		function add_remove_handlerAll(target, destination, select_1, select_2,AddORRemove) {
			...
			target.remove();
			 ...
			 if((AddORRemove ==1 || AddORRemove ==2) && document.getElementById(select_1).length == 0){
			 clear_button2(select_2);
			 }
		...
		function search_filter(selector) {
			$(selector + ' .listbox_search input#search_listbox').keyup(
					function() {
			...
			$(search_selector + ' .listbox_search input#search_listbox').focus(
					function() {
						...
						var data_search = $(div_id + ' select#' + select_id)
								.attr('data-search').toString().toLowerCase();
						if (val == data_search)
							$(this).val('');
					});
		...
		function clear_button2(selector) {
			$(selector + ' ul > li').each(function() {
				$(this).show();
			});
			$("#listbox_"+selector+"_wrapper > div > span").click();
			refresh_list();
		}
		...
		function refresh_list() {
			...
			$(div_id + ' .source_wrapper li.listbox_option').filter(":even")
					.addClass('even');
			$(div_id + ' .source_wrapper li.listbox_option').filter(":odd")
					.addClass('odd');
			$(div_id + ' .destination_wrapper li.listbox_option').filter(
					":even").addClass('even');
