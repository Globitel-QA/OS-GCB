...
<!-- bootstrap 3  -->
<link href="../../../theme-rtl/assets/global/plugins/bootstrap/css/bootstrap-rtl.min.css"
	rel="stylesheet" type="text/css" />

<link href="../../../theme-rtl/assets/global/plugins/bootstrap-switch/css/bootstrap-switch-rtl.min.css"
	rel="stylesheet" type="text/css" />
...
<body>
	...
	<script
		src="../node_modules/bootstrap-select/dist/js/bootstrap-select.js"></script>
	...
	<script>
		...
		var parameters;
		var OpenFAlrarm = 0; // to check if page opend from alarm config management or rules managment 
		...
		function CreateCondition() {
				...
				success : function(data) {
					if (data != 0) {
						...
						$.each(Obj, function(key, value) {
							StrEvent += '{"label": "' + value + '", "value": "'
									+ value + '"},';

						});
						$('#listEventsID').html(Str);
						//$('#listEventsID').select2();
					} else {
					}
			...
			FilterData = JSON
					.parse('[{"id":"MSISDN","field":"msisdn","label":"'
							+ window.parent.jQuery.i18n.prop('MSISDNsList_MSISDN') +'","type":"integer","input":"text","size":20 ,"data": { "class": "form-control" }'
							+ ',"operators": ["equal", "not_equal", "contains" ]  }, '

							+ '{"id":"CellID","field":"event.details.new_ci","label":"'
							+ window.parent.jQuery.i18n.prop('CellsIDs_CellID') +'","type":"string","input":"select", "multiple": "true","placeholder": "'
							+ window.parent.jQuery.i18n.prop('CellsIDs_SelectCellID') +'" , "values": ['
							+ StrCells
							+ '] '
							+ ',"operators": ["in"]}, '

							+ '{"id":"ZoneID","field":"event.details.new_zid","label":"'
							+ window.parent.jQuery.i18n.prop('Zones_Zone') +'","type":"string","input":"select", "multiple": "true","placeholder": "'
							+ window.parent.jQuery.i18n.prop('Zones_SelectZone') +'" , "values": ['
							+ StrZones
							+ '] '
							+ ',"operators": ["in"]}, '

							+ '{"id":"DurationID","field":"event.details.duration","label":"'
							+ window.parent.jQuery.i18n.prop('Date_Duration') +'","type":"string","input":"select", "nb_inputs": "3" ,"placeholder": "'
							+ window.parent.jQuery.i18n.prop('Date_SelectDuration') +'" , "values": ['
							+ StrDuration
							+ '] '
							+ ',"operators": ["between"]}, '

							+ '{"id":"calling_number","field":"event.details.calling_number","label":"'
							+ window.parent.jQuery.i18n.prop('jQueryBuilder_CallingNumber') +'","type":"integer","input":"text","size":20,"data": { "class": "form-control" } '
							...
							+ window.parent.jQuery.i18n.prop('jQueryBuilder_CalledNumber') +'","type":"integer","input":"text","size":20,"data": { "class": "form-control" } '
							+ ',"operators": ["equal", "not_equal" ]}, '
							+ '{"id":"event.types","field":"event.type","label":"'
							+ window.parent.jQuery.i18n.prop('EventMonitoring_EventType') +'","type":"string","input":"select","placeholder": "'
							+ window.parent.jQuery.i18n.prop('EventMonitoring_SelectEventType') +'" , "values": ['
							+ StrEvent
							+ '] '
							+ ',"operators": ["equal", "not_equal"] } ] ');
			...
			$('#builder')
					.on(
							...
							function(e, rule) { // function to make anything after created element 

								if (rule.filter.input == "select"
										|| rule.filter.type == "string") { // check if select make search on select use select2 libaray 
								...
								if (rule.filter.plugin == 'selectize') { // set all select same size 
									...
									rule.$el.find('.rule-value-container').css(
											'min-width', '200px').find(
											'.selectize-control').removeClass(
											'form-control');
		...
		$('.parse-mongo').on(
				...
				function() {
					...
					CreateFilter(); // for make validation 
					// drwa query on html
		...
		$('.destroy').on('click', function(e) {
			e.preventDefault();
			window.parent.ResetCondition(); // create this way for confirm message 
		...
		function CreateFilter() {
			...
			let MingoQuery = new mingo.Query(RuleQuery); // MingoQuery is convert RuleQuery to mingo query to test if match or not match 
			let DataObject = [];
			if (MingoQuery.test(DataObject)) { // to check is match or not match 
				$('#Match').text("Query is match");
			} else {
				$('#Match').text("Query isn't match");
			}

			//filter collection with find()
			let cursor = query.find(DataObject)
		...
		function IsFormComplete() { // TypeSave == 0 save  TypeSave = 1 from save and copy subscriber alarm only
			...
			if (FormOk == true
				...
				window.parent
						.AlertFunction(window.parent.jQuery.i18n.prop('jQueryBuilder_AlertAtLeastCondition'));
				FormOk = false;
			}
