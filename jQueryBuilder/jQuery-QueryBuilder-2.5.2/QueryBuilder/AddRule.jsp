...
<!-- Select 4.0.12  CSS  -->
<link
	href="../../../theme/assets/global/plugins/select2/css/select2.min.css"
	rel="stylesheet" type="text/css" />
...
<!-- links for library  -->
	...
	<script>
		...
		function CreateCondition() {
			...
			FilterData = JSON
					.parse('[{"id":"MSISDN","field":"msisdn","label":"'
							+ window.parent.jQuery.i18n.prop('MSISDNsList_MSISDN') +'","type":"string","input":"text","size":20 ,"data": { "class": "form-control" }'
							+ ',"operators": ["equal", "not_equal", "contains" ]  }, '

							+ '{"id":"CellID","field":"event.details.location.new_ci","label":"'
							+ window.parent.jQuery.i18n.prop('CellsIDs_CellID') +'","type":"string","input":"select", "multiple": "true" , "values": ['
							+ StrCells
							+ '] '
							+ ',"operators": ["in"]}, '

							+ '{"id":"ZoneID","field":"event.details.new_zid","label":"'
							+ window.parent.jQuery.i18n.prop('Zones_Zone') +'","type":"string","input":"select", "multiple": "true" , "values": ['
							+ StrZones
							+ '] '
							+ ',"operators": ["in"]}, '
			...
			setTimeout(() => { 
				  $(".dropdown.bootstrap-select.bs3 , .form-control").css("width","130px");
				  // deleting all rows when showing condition first time
		...
		function ResetForm(){ // create this function for reset from admin tree for full width screen
			//$("#txtName").val("");
			//$("#listProtocol").val(-1);
		...
		$('.btn.btn-xs.btn.blue').click(function() {
			  ...
			  if( $('.btn.btn-xs.btn-danger').length > 0){
		  		window.parent.$("#FrameContent3").height(parseInt(window.parent.$("#FrameContent3").height()+30))
